...
import { ArrowUpRight, ArrowDownRight, Calendar, Filter, BarChart2, Target, Clock, Mail, MessageSquare, Send, Smartphone, ChevronRight, Users, Loader2, Search, RefreshCw, Download, Eye, TrendingUp, Activity } from 'lucide-react';
...
const TAB_WHATSAPP_ANALYTICS = 'WhatsApp Analytics';
...
// WhatsApp Campaign interfaces and data
...
const EngagementDashboard = () => {
  ...
  const [whatsappCampaigns, setWhatsappCampaigns] = useState<WhatsAppCampaign[]>(mockWhatsAppCampaigns);
  ...
  const filteredWhatsAppCampaigns = whatsappCampaigns.filter(campaign => {
    const matchesSearch = campaign.name.toLowerCase().includes(whatsappSearchTerm.toLowerCase()) ||
                         campaign.templateName.toLowerCase().includes(whatsappSearchTerm.toLowerCase()) ||
                         campaign.id.toLowerCase().includes(whatsappSearchTerm.toLowerCase());
    const matchesStatus = whatsappStatusFilter === 'all' || campaign.status === whatsappStatusFilter;
    return matchesSearch && matchesStatus;
  });

  const loadWhatsAppCampaignDetails = async (campaignId: string) => {
    setIsWhatsappLoading(true);
    try {
      // In production, this would fetch from your API
      const campaign = whatsappCampaigns.find(c => c.id === campaignId);
      if (campaign) {
        setSelectedWhatsAppCampaign(campaign);
      }
    } catch (error) {
      console.error('Failed to load WhatsApp campaign details:', error);
    } finally {
  ...
  const refreshWhatsAppCampaigns = async () => {
    ...
    try {
      ...
      console.log('Refreshing WhatsApp campaigns...');
      // Simulate API call
      await new Promise(resolve => setTimeout(resolve, 1000));
    } catch (error) {
      console.error('Failed to refresh WhatsApp campaigns:', error);
    } finally {
      setIsWhatsappLoading(false);
    }
  };

  const getWhatsAppStatusBadgeVariant = (status: string) => {
    switch (status) {
    ...
    <div className="p-6 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-2">
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mt-0">View and analyze your email campaign performance</p>
        </div>
        </div>
      ...
      <div className="flex gap-8 border-b border-gray-200 mb-4">
        ...
        <button
          className={`pb-2 font-medium ${selectedTab === TAB_WHATSAPP_ANALYTICS ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-indigo-600'}`}
          onClick={() => setSelectedTab(TAB_WHATSAPP_ANALYTICS)}
        >
          WhatsApp Analytics
        </button>
      </div>
      ...
      {selectedTab === TAB_WHATSAPP_ANALYTICS && (
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="flex items-center gap-2">
                    ...
                    WhatsApp Campaigns ({filteredWhatsAppCampaigns.length})
                  </CardTitle>
                  <Button 
                    variant="outline" 
                    size="sm"
                    onClick={refreshWhatsAppCampaigns}
                    disabled={isWhatsappLoading}
                  >
                    <RefreshCw className={`w-4 h-4 ${isWhatsappLoading ? 'animate-spin' : ''}`} />
                  </Button>
                </div>
              ...
              <CardContent className="p-0">
                <div className="max-h-96 overflow-y-auto">
                  {filteredWhatsAppCampaigns.map((campaign) => (
                    <div
                      key={campaign.id}
                      onClick={() => loadWhatsAppCampaignDetails(campaign.id)}
                      className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors ${
                        selectedWhatsAppCampaign?.id === campaign.id ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''
                      }`}
                    >
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-medium text-sm truncate">{campaign.name}</h3>
                        <Badge variant={getWhatsAppStatusBadgeVariant(campaign.status)}>
                          {campaign.status}
                        </Badge>
                      </div>
                  ...
                  {filteredWhatsAppCampaigns.length === 0 && (
                    <div className="p-8 text-center text-gray-500">
                      ...
                      <p>No WhatsApp campaigns found</p>
            ...
            {selectedWhatsAppCampaign ? (
              <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="flex items-center gap-4 mt-2 text-sm text-gray-600">
                          <span>ID: <code className="bg-gray-100 px-1 rounded">{selectedWhatsAppCampaign.id}</code></span>
                          <span>Template: <code className="bg-gray-100 px-1 rounded">{selectedWhatsAppCampaign.templateName}</code></span>
                          <Badge variant={getWhatsAppStatusBadgeVariant(selectedWhatsAppCampaign.status)}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <Card>
                    <CardContent className="p-4 text-center">
                      <div className="text-2xl font-bold text-blue-600">{selectedWhatsAppCampaign.totalContacts}</div>
                  ...
                  <Card>
                    <CardContent className="p-4 text-center">
                      <div className="text-2xl font-bold text-green-600">{selectedWhatsAppCampaign.sentCount}</div>
                      <div className="text-sm text-gray-600">Sent</div>
                      <div className="text-xs text-green-600">
                        {selectedWhatsAppCampaign.totalContacts > 0 ? Math.round((selectedWhatsAppCampaign.sentCount / selectedWhatsAppCampaign.totalContacts) * 100) : 0}%
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-4 text-center">
                      <div className="text-2xl font-bold text-purple-600">{selectedWhatsAppCampaign.deliveredCount}</div>
                      <div className="text-sm text-gray-600">Delivered</div>
                      <div className="text-xs text-purple-600">
                        {selectedWhatsAppCampaign.sentCount > 0 ? Math.round((selectedWhatsAppCampaign.deliveredCount / selectedWhatsAppCampaign.sentCount) * 100) : 0}%
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-4 text-center">
                      <div className="text-2xl font-bold text-indigo-600">{selectedWhatsAppCampaign.readCount}</div>
                      <div className="text-sm text-gray-600">Read</div>
                      <div className="text-xs text-indigo-600">
                        {selectedWhatsAppCampaign.deliveredCount > 0 ? Math.round((selectedWhatsAppCampaign.readCount / selectedWhatsAppCampaign.deliveredCount) * 100) : 0}%
                      </div>
                    </CardContent>
                  </Card>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Card>
                    <CardContent className="p-4">
                      ...
                      <div className="text-2xl font-bold text-orange-600">{selectedWhatsAppCampaign.ctr}%</div>
                      <div className="text-sm text-gray-600">{selectedWhatsAppCampaign.clickCount} clicks</div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-4">
                      ...
                      <div className="text-2xl font-bold text-red-600">{selectedWhatsAppCampaign.failedCount}</div>
                      <div className="text-sm text-gray-600">
                        {selectedWhatsAppCampaign.totalContacts > 0 ? Math.round((selectedWhatsAppCampaign.failedCount / selectedWhatsAppCampaign.totalContacts) * 100) : 0}% failure rate
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-4">
                      ...
                      <div className="text-2xl font-bold text-gray-600">${selectedWhatsAppCampaign.estimatedCost}</div>
                      <div className="text-sm text-gray-600">
                        ${(selectedWhatsAppCampaign.estimatedCost / Math.max(selectedWhatsAppCampaign.sentCount, 1)).toFixed(3)} per message
                      </div>
                    </CardContent>
                  </Card>
                </div>
                ...
                <Tabs defaultValue="timeline" className="w-full">
                  <TabsList>
                    ...
                    <TabsTrigger value="contacts">Contact Details ({selectedWhatsAppCampaign.contacts.length})</TabsTrigger>
                  </TabsList>
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-sm text-gray-600">{new Date(selectedWhatsAppCampaign.createdAt).toLocaleString()}</div>
                            </div>
                          </div>
                          ...
                          {selectedWhatsAppCampaign.startedAt && (
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="text-sm text-gray-600">{new Date(selectedWhatsAppCampaign.startedAt).toLocaleString()}</div>
                              </div>
                            </div>
                          )}
                          ...
                          {selectedWhatsAppCampaign.completedAt && (
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="text-sm text-gray-600">{new Date(selectedWhatsAppCampaign.completedAt).toLocaleString()}</div>
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        {selectedWhatsAppCampaign.contacts.length > 0 ? (
                          <div className="overflow-x-auto">
                            <Table>
                              ...
                              <TableBody>
                                {selectedWhatsAppCampaign.contacts.map((contact, index) => (
              ...
              <Card className="h-96 flex items-center justify-center">
                <div className="text-center text-gray-500">
                  ...
                  <h3 className="text-lg font-medium mb-2">Select a WhatsApp Campaign</h3>
                  <p>Choose a campaign from the list to view detailed analytics and read status</p>
                  <p className="text-sm mt-2">Real delivery reports will reflect here when messages are read</p>
                </div>
              </Card>
            )}
          </div>
