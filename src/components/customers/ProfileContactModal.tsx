...
const callObjectives = [
  {
    ...
    description: 'Regular check-in on customer satisfaction',
    questions: [
      'How has your experience been with our services?',
      'Is there anything we can improve?',
      'Do you have any upcoming needs we can help with?',
      'Any feedback on our recent updates?'
    ]
  },
  {
    id: 'support',
  ...
  },
  {
    ...
    description: 'Discuss contract renewal and terms',
    questions: [
      'How has the service performed for you this year?',
      'Are you planning to renew your contract?',
      'Any changes needed for the next term?',
      'Questions about pricing or new features?'
    ]
  }
];
...
export const ProfileContactModal: React.FC<ProfileContactModalProps> = ({
  isOpen,
  onClose,
  profile
}) => {
  ...
  const getRecommendedObjective = () => {
    switch (customer.status.toLowerCase()) {
      case 'prospect': return 'check_in';
      case 'active': return 'support';
      case 'inactive': return 'renewal';
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <Select value={objective} onValueChange={setObjective}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select contact objective..." />
                  </SelectTrigger>
                  <SelectContent>
              ...
              {/* Contact Notes */}
              <div className="space-y-2">
                <Label htmlFor="notes">Contact Notes & Agenda</Label>
                <Textarea
                  id="notes"
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="Enter contact notes, agenda items, or key points to discuss..."
                  rows={6}
                />
              </div>
              ...
              {(callType === 'immediate' && !isCallActive && callStartTime) && (
                <div className="space-y-2">
                  ...
                  <Select value={callOutcome} onValueChange={setCallOutcome}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select contact outcome..." />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t">
                ...
                >
                  {callType === 'immediate' ? (
                    <>
                      <Phone className="w-4 h-4 mr-2" />
                      {isCallActive ? 'Save Contact Log' : 'Start Contact'}
                    </>
                  ) : callType === 'scheduled' ? (
                    <>
